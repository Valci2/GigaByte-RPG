...
void responderOpcao(int resposta) {
    switch (resposta) {
        case 1:
            ...
            break;
        case 2:
            std::cout << "Poxa... voce sabe que o nome do jogo eh *Albion Online* e mesmo assim clicou em Gigabyte." <<
                    std::endl;
            break;
        case 3:
...
Personagem apresentacao() {
    ...
    std::cout << "preciso criar um objeto da classe Personagem. Para isso, preciso que voce forneca seu nome." <<
            std::endl;
...
void historia01() {
    std::cout << "No glorioso, caotico e totalmente instavel mundo de GigaByte — um mundo onde componentes eletronicos"
            << std::endl;
    std::cout << "ganham vida, personalidades duvidosas e vontade propria de te ver sofrer. De relays revoltados ate" <<
            std::endl;
    std::cout << "CPUs com delirio de grandeza, cada um mais insuportavel que o outro." << std::endl;
    delay(5);

    linha();
    std::cout << "Seu objetivo? Enfrentar essa trupe de sucata viva, subir de nivel e, com sorte (e um milagre)," <<
            std::endl;
    std::cout << "encarar o Boss Supremo: o lendario PC. Isso mesmo, o PC. Aquele que liga quando quer, reinicia no" <<
            std::endl;
    std::cout << "meio da luta e ainda joga a culpa no driver da placa de video." << std::endl;
    delay(5);

    linha();
    std::cout << "Domine. Capture. Aprenda. (Quer dizer, 'capturar' e modo de dizer mesmo... o dev ate pensou em" <<
            std::endl;
    std::cout <<
            "colocar mecanica de captura, mas ai lembrou que tinha vida para viver, abriu o iFood e nunca mais voltou nessa"
            << std::endl;
    ...
    linha();
    std::cout <<
            "Vou te apresentar o Lobby eh nele que voce pode fazer upgrades, descansar, escolher as fases, ver os status, Etc."
            << std::endl;
